...
void ReadRelayAll(BYTE * bState);
BYTE GetIPBroadcast(char * INLocalIP, char * OUTBroadIP);
...
/*	static WORD bSendCycles = SEND_CYCLES;*/
	...
	while (1)
	{
		...
		{
			SendPackageFindMe();
                        //Serial.println("BroadcastMessage");
			bSendCycles = 0U;
		}
		else
		{
		...
		*/
		...
		if (size > 0)
		{
			...
			psRcvPackage->FromCom = TETHERNET;
...
BYTE ProcessPackage(sPackage * psRcvPackage, TYPECOM FromCOM)
{
	...
	case CMD_RELAY_WRITE:
	{
		...
		ReadRelayAll(&bRelayState);
		psRcvPackage->bRXData[0] = bRelayState; 
		psRcvPackage->bNumData = 1;
	}
	break;
	...
	case CMD_GET_IP:
	{
		...
		psRcvPackage->bRXData[0] = myIP[0];
		psRcvPackage->bRXData[1] = myIP[1];
		psRcvPackage->bRXData[2] = myIP[2];
		psRcvPackage->bRXData[3] = myIP[3];


		psRcvPackage->bNumData = 4;
	}
	break;
...
#endif
	SendPackage(psRcvPackage, psRcvPackage->FromCom);
...
BYTE SendPackageFindMe( void )
{
	...
	snprintf(IPAddr, sizeof(IPAddr), "%d.%d.%d.%d", MyIP[0], MyIP[1], MyIP[2], MyIP[3]);
	...
	bBuffer[4] = 0;
	bBuffer[5] = 0;
	bBuffer[6] = sizeof(IPAddr);

	memcpy(&bBuffer[7], IPAddr, sizeof(IPAddr));
	...
	udp.beginPacket(MyIP, 5001);
	bSentData = udp.write(bBuffer, sizeof(IPAddr)+9);
	udp.endPacket();
...
BYTE GetIPBroadcast(char * INLocalIP, char * OUTBroadIP)
{
	char * InitPos = INLocalIP;
	char * Pos = NULL;
	BYTE bCount = 0;
	int SizeToCopy;

	Pos = strrchr(INLocalIP, '.');
	...
	SizeToCopy = Pos - InitPos - 1;

	memcpy(OUTBroadIP, INLocalIP, SizeToCopy);

	strcat(OUTBroadIP, "255");
